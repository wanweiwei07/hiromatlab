function interair = computepushair(objfile, palmfile, fgr1file, fgr2file)
% compute possible push of the object in the air
...
% the standard object pose in the air and its associated grasps
    ...
    interair.stablemesh.pcd = cvtpcd(interair.stablemesh.verts', interair.stablemesh.faces', 10000);
    ...
    nhandorientation = size(handorientation, 1);
    
    % for each face
    totalgrasps = [];
    for i = 1:nface
        [isavailable, projectedpoint] = projPointOnPolygon3d(interair.objcenter, faces(i));
        if(isavailable)
            thisfacenorm = normsface(i, :);
            for j = 1:nhandorientation
                possiblegrasp.handx = handorientation(j,:);
                possiblegrasp.handy = cross(handz, thisfacenorm);
                possiblegrasp.handz = cross(handx, handy);
                possiblegrasp.fgrcenter = projectedpoint';
                possiblegrasp.tcp = possiblegrasp.fgrcenter+possiblegrasp.handx*0.15;
                possiblegrasp.fgr1 = possiblegrasp.tcp;
                possiblegrasp.fgr2 = possiblegrasp.tcp;
                possiblegrasp.fgrdist = 0;
    ...
    for i = 1:npps
        ...
        fgr1pnts = sampletriangle(fgr1faceverts(1,:), fgr1faceverts(2,:), fgr1faceverts(3,:), 1000);
        if isempty(fgr1pnts)
            continue;
        end
...
%             patch('vertices', fgr2faceverts, 'faces', [1,2,3], 'facecolor', [1,0.8,0.8], 'facealpha',0.9);
            ...
            for k = 1:8
                ...
                possiblegrasp.tcp = fgrcenter'+possiblegrasp.handx*0.15;
