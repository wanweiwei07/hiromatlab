...
% weiwei: the direction only works for convex objects
% 
...
%     v2 = normalizeVector3d(v2);
    ...
    for i = 1:length(faces)
        ...
        % weiwei
        % use end to avoid colinear case
        v2 = nodes(face(end),1:3) - nodes(face(1),1:3);
...
%         v2 = normalizeVector3d(v2);
    ...
    % 1. calc vertex center
    nnormals = size(normals, 1);
    vertex = [];
    for i = 1:nnormals
        vertex = [vertex;nodes(faces{i}, :)];
    end
    cvert = mean(vertex);
    % 2. compare
    for i = 1:nnormals
        % flip normal (only for convex objects)
        vertex = nodes(faces{i}, :);
        mvert = mean(vertex);
        v = mvert - cvert;
        d  = sum(v.*normals(i, :));
        if d < 0
