function polys = intersectPlaneMeshFast(plane, v, f, edges, faceEdges)
...
polys = {};
while sum(validEdgeInds) ~= 0
    % start new polygon
    
    % start at any edge, mark it as current
    % find first none zero element
    nonzeroInds = find(validEdgeInds);
    startEdgeIndex = validEdgeInds(nonzeroInds(1));
...
%     processedEdge(currentEdgeIndex) = true;
    ...
    % make close; weiwei
    polyEdgeInds = [polyEdgeInds startEdgeIndex];
    poly = intersectionsPoints(polyEdgeInds, :);
    
    polys = [polys, {poly}]; %#ok<AGROW>
    
    for i = 1:size(polyEdgeInds')-1
        validEdgeInds(validEdgeInds==polyEdgeInds(i)) = 0;
    end
