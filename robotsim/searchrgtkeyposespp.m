function [isdone, keyposes] = searchrgtkeyposespp(hironx, graphstates)
...
% date: 20160721
    ...
    for i = 1:npathnode-1
        ...
        % prg-preg
        firstgraspparampp = rgp.states{path(i)}.ikprggrp(3).graspparams(grasppairs(1,1));
        firstgraspparamidpp = rgp.states{path(i)}.ikprggrp(3).graspparamids(grasppairs(1,1));
        secondgraspparampp = rgp.states{path(i+1)}.ikprggrp(3).graspparams(grasppairs(1,2));
        secondgraspparamidpp = rgp.states{path(i+1)}.ikprggrp(3).graspparamids(grasppairs(1,2));
        ...
        % firt pp, hand open
        keyposes{end+1,1} = rgp.states{path(i)}.ikprggrp(3);
        keyposes{end,1}.graspparams=firstgraspparampp;
        keyposes{end,1}.graspparamids=firstgraspparamidpp;
        ...
        keyposes{end,1}.graspparams(1).handstate = 'open';
        % second prgpp
        keyposes{end+1,1} = rgp.states{path(i+1)}.ikprggrp(3);
        keyposes{end,1}.graspparams=secondgraspparampp;
        keyposes{end,1}.graspparamids=secondgraspparamidpp;
        keyposes{end,1}.graspparams(1).handstate = 'open';
    end
end
